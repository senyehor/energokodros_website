...
class UserRoleApplicationDecision(FormView):
    ...
    template_name = 'administrator/user_role_application.html'
    form_class = UserRoleApplicationRequestsDecisionForm
    success_url = reverse_lazy('users_roles_applications')

    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['user_has_no_roles'] = _check_user_has_no_roles(self.__get_application_request().user)
        return ctx
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        self.controller = UserRoleApplicationReviewController(  # noqa
            form,
            self.__get_application_request(),
            self.request,
        )
    ...
    def form_invalid(self, form: UserRoleApplicationRequestsDecisionForm):
        if not (is_approved := self.application_approved):
            # case when admin decided to decline application,
            # so we do not care about any form validation
            self.controller.process_depending_on_decision(is_approved)
        return super().form_invalid(form)

    def form_valid(self, form: UserRoleApplicationRequestsDecisionForm):
        self.controller.process_depending_on_decision(self.application_approved)
