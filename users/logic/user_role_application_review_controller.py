...
class UserRoleApplicationReviewController:
    def __init__(self, application_decision_form: UserRoleApplicationRequestsDecisionForm,
        ...
        self.__application_decision_form = application_decision_form
        self.__request = request
        self.__is_approved = is_approved

    @method_decorator(atomic)
    def process_depending_on_decision(self):
        if self.__is_approved:
    ...
    def __add_message_for_decision(self):
        ...
        messages.success(self.__request, message)

    def __create_user_role(self):
        user_role_without_user: UserRole = self.__application_decision_form.instance
        user_role_without_user.user = self.__application.user
    ...
    def __notify_on_role_application_decision(self):
        ...
        if message_from_admin := self.__application_decision_form.get_message_for_user():
            message += f'<br>Повідомлення від адміністратора:<br> {message_from_admin}'

        try_send_email_add_warning_if_failed(
            self.__request,
            self.__application.user.email,
            subject,
            mark_safe(message)
        )
